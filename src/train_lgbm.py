...
LGB_MDOEL_PARAMS = config.LGB_MDOEL_PARAMS
LGB_TRAIN_PARAMS = config.LGB_TRAIN_PARAMS
...
def train_1fold(preds, preds_test, importance, trn_idx, val_idx, X_train, y_train, X_test):
    ...
    d_train = lgb.Dataset(x_trn, label=y_trn)
    ...
    importance_fold = model.feature_importance(importance_type='gain')
    for i, f in enumerate(fnames):
        if f not in importance:
            importance[f] = 0
        importance[f] += importance_fold[i]
...
if __name__ == "__main__":
    ...
    if skip_fr is False:
        ...
        X_train = load_from_pkl(f'X_train_task1_lgbm_t2.pkl')
        X_test = load_from_pkl(f'X_test_task1_lgbm_t2.pkl')
    ...
    with t.timer(f'train LGBM'):
        ...
        if TARGET_TASK == '1':
            ...
            preds = np.zeros((len(X_train), LGB_MDOEL_PARAMS['num_class']))
            preds_test = np.zeros((len(X_test), LGB_MDOEL_PARAMS['num_class']))
        ...
        for fold_idx in range(FOLD_NUM):
            ...
            preds, preds_test, importance = train_1fold(
                ...
                trn_idx, val_idx, X_train, y_train, X_test
            )
    ...
    if TARGET_TASK == '1':
        ...
        preds_test_label = np.argmax(preds_test, axis=1)
        cv = metrics.accuracy_score(y_train, preds_label)
        logging.info(f'CV Multiclass Acc: {cv}')
        preds_label = preds_label + 1
        preds_test_label = preds_test_label + 1
